...
async function httpGet(path: string, params?: Record<string, string | number>, skipCache = false): Promise<any> {
  const qs = params ? new URLSearchParams(Object.entries(params).map(([k, v]) => [k, String(v)])).toString() : ''
  const url = `${BASE_URL}${path}${qs ? `?${qs}` : ''}`
  const cacheKey = `${path}:${qs}`
  
  // 1️⃣ CACHE CHECK (unless skipCache or fresh requested)
  if (!skipCache) {
    const cached = binanceCache.get(cacheKey)
    if (cached !== null) {
      console.log(`[CACHE_HIT] ${path}`)
      return cached
    }
  }
  
  // 2️⃣ REQUEST COALESCER (merge duplicate in-flight requests)
  return requestCoalescer.fetch(cacheKey, async () => {
    ...
    try {
      ...
      // 4️⃣ CACHE STORAGE (unless skipCache)
      if (!skipCache) {
        binanceCache.set(cacheKey, data, path)
      }
...
async function httpGetCached(path: string, params: Record<string, string | number> | undefined, _ttlMs: number, fresh = false): Promise<any> {
  // Use cache by default, but allow force bypass with 'fresh' parameter
  return httpGet(path, params, fresh)
}
...
async function getTopLosersUsdtSymbols(n: number, fresh = false): Promise<string[]> {
  const data = await withRetry(() => httpGetCached('/fapi/v1/ticker/24hr', undefined, (config as any).cache?.ticker24hMs ?? 30000, fresh), config.retry)
  const entries = Array.isArray(data) ? data : []
  const filtered = entries.filter((e: any) => e?.symbol?.endsWith('USDT'))
  const sorted = filtered.sort((a: any, b: any) => {
    const pa = Number(a.priceChangePercent)
    const pb = Number(b.priceChangePercent)
    if (pa !== pb) return pa - pb  // ascending sort (největší ztráty první)
    // tie-break by volume
    const va = Number(a.quoteVolume)
    const vb = Number(b.quoteVolume)
    if (vb !== va) return vb - va
...
export async function buildMarketRawSnapshot(opts?: { universeStrategy?: 'volume'|'gainers'|'losers'|'overheat'; desiredTopN?: number; includeSymbols?: string[]; fresh?: boolean; allowPartial?: boolean }): Promise<MarketRawSnapshot> {
  const t0 = Date.now()
  ...
  console.error(`[BUILD_SNAPSHOT] baseList.length=${baseList.length}, first5=${baseList.slice(0,5).join(', ')}`)
