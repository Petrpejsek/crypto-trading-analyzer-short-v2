...
export function isAuditEnabled(): boolean {
  try {
    const v = String(process.env.PROFIT_TAKER_AUDIT || '').toLowerCase()
    return v === '1' || v === 'true' || v === 'yes'
  } catch { return false }
}

function getAuditDir(): string {
  return path.resolve(process.cwd(), 'runtime', 'audit', 'profit_taker')
}
...
export function appendAudit(record: AuditRecord): void {
  ...
  try {
    ...
    fs.appendFile(file, line, (err) => { if (err) { try { console.error('[PT_AUDIT_APPEND_ERR]', err.message || err) } catch {} } })
  } catch (e) {
    try { console.error('[PT_AUDIT_WRITE_ERR]', (e as any)?.message || e) } catch {}
  }
}
