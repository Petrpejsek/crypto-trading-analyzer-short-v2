...
export function isAuditEnabled(): boolean {
  try {
    const v = String(process.env.ENTRY_UPDATER_AUDIT || '').toLowerCase()
    return v === '1' || v === 'true' || v === 'yes'
  } catch { return false }
}

function getAuditDir(): string {
  return path.resolve(process.cwd(), 'runtime', 'audit', 'entry_updater')
}
...
export function appendAudit(record: AuditRecord): void {
  ...
  try {
    ...
    fs.appendFile(file, line, (err) => { if (err) { try { console.error('[EU_AUDIT_APPEND_ERR]', err.message || err) } catch {} } })
  } catch (e) {
    try { console.error('[EU_AUDIT_WRITE_ERR]', (e as any)?.message || e) } catch {}
  }
}
